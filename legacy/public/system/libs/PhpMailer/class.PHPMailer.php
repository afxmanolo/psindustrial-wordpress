...
//    }
  	...
  	// ARL: This had to be done in order to work for now. (phpmailer.lang-en.php)
  ...
  function MsgHTML($message) {
    ...
    if(isset($images[2])) {
      foreach($images[2] as $i => $url) {
      	preg_match_all('/^(https|http|ftp):\/\/(www\.)?(.+)\.(com|net|org|es|mx|ar)(es|mx|ar)*(\/(([[:alnum:]]|-|_|\+|\%|\.|#)+)*(\/)*)*((\?(([[:alnum:]]|-|_)+)=([[:alnum:]]|[[:space:]]|-|_|\+|\%|\.|#)+)((&(([[:alnum:]]|-|_)+)=(([[:alnum:]]|[[:space:]]|-|_|\+|\%|\.|#))+)*)*)*$/', $url, $urls);
      	...
      	if(!isset($urls[0])){
	        ...
	        $ext       = $fileParts[1];
	        $mimeType  = $this->_mime_types($ext);debug($mimeType);
