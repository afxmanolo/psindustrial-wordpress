...
//		}
	...
	*/
	function send($data, $timeout=0, $response_timeout=30, $cookies=NULL) {
		...
		while ($this->tryagain) {
			...
			if ($tries++ < 2) {
				// make connnection			    
				$conexion=$this->connect($timeout, $response_timeout);
				if (!$conexion){
					return false;
				}
				
				// send request
				$req=$this->sendRequest($data, $cookies);
	...
	* @access   public
	*/
	function setSOAPAction($soapaction,$prefix="") {	
	   
		$this->setHeader('SOAPAction', '"' .$prefix.$soapaction . '"');
	}
	...
	*/
	function sendRequest($data, $cookies = NULL) {
	  ...
	  if ($this->io_method() == 'socket') {	  	
		...
		$respuesta =fputs($this->fp, $this->outgoing_payload);
	...
	*/
	...
	if ($this->io_method() == 'socket') {	  
	    ...
	    while (!isset($lb)){
			...
			$tmp = fgets($this->fp, 256);
			/*if($_SERVER['REMOTE_ADDR'] == '189.203.177.63'){
			    var_dump($this->scheme);die;
			}*/
			/*echo "2938"; 
			var_dump($tmp); die;*/  
			$tmplen = strlen($tmp);
...
//		}
		...
		if(isset($this->incoming_headers['content-encoding']) && $this->incoming_headers['content-encoding'] != ''){
			...
			//aqui entraba cuando cargaba lento 09/06/2011 
	...
	*/
	function nusoap_parser($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		...
		if(!empty($xml)){
			...
			// Parse the XML file.
			if ($_REQUEST['cmd']=="existencias" || $_REQUEST['cmd']== 'precios'){
				$parser = false;
			}else{
				$parser = true;
			}
			if(!xml_parse($this->parser,$xml,true) && $parser){
			    // Display an error message.
...
class nusoap_client extends nusoap_base  {
	 ...
	 */
	var $faultdetail;
	var $soap_action_prefix="";

	function setSoapActionPrefix($action_pref){
		$this->soap_action_prefix=$action_pref; 
	}
	function getSoapActionPrefix(){
		return $this->soap_action_prefix;
	}
	...
	* @access   public
	*/
	function nusoap_client($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30, $authtype = 'basic'){
		parent::nusoap_base();
	...
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=30) {	
		...
		switch(true){
			...
			case ereg('^http',$this->endpoint):
				...
				$http->setSOAPAction($soapaction,$this->getSoapActionPrefix());	
				if($this->proxyhost && $this->proxyport){
