...
## Botón flotante de WhatsApp — 2026-09-25

`Settings::whatsapp_url()` ya existía (usada sólo en la pantalla de configuración, nunca en el theme) -- causa real de por qué el botón nunca aparecía. Se reforzó su normalización (elimina cualquier carácter no numérico antes de construir `https://wa.me/...`, en vez de asumir el formato estricto que sólo `sanitize()` garantiza al guardar) y se añadió `theme/inc/whatsapp.php`, un único hook `wp_footer` (no una plantilla copiada) que renderiza el botón sólo cuando `whatsapp_url()` devuelve un enlace real. Diseño fiel a `legacy/public/css/custom.css` (`.whatsapp`: 60×60px, 20px de los bordes, `#25d366`, circular, z-index:100), con SVG local en vez de Font Awesome. Pruebas: `tests/whatsapp-button.php` (nuevo, 104/104) -- habilitado/deshabilitado, número vacío, normalización con espacios/guiones/paréntesis, mensaje codificado, aparición global (9+ tipos de página), atributos de accesibilidad, ausencia de número hardcodeado.

**Incidente de infraestructura (no relacionado con este cambio):** durante la verificación, MySQL y Apache de Laragon se detuvieron inesperadamente (causa no confirmada, posiblemente ligada a una gestión de procesos anterior en esta misma sesión) y se reiniciaron manualmente. Tras el reinicio, dos archivos de test preexistentes (`tests/batch-recovery-model.php`, `tests/datasheets-guard-fix.php`) quedaron bloqueados para escritura a nivel de SO/antivirus (confirmado con Bash y PowerShell nativo por separado; la misma ruta exacta rechaza cualquier escritura mientras que el mismo contenido bajo otro nombre se escribe sin problema) -- pendiente de que el usuario revise cuarentena de su antivirus para esos dos nombres exactos. La suite de regresión mostró además bloqueos intermitentes en archivos no relacionados con este cambio durante la misma sesión, consistentes con inestabilidad residual del entorno tras el incidente, no con este código.
